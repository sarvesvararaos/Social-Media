import { PostsModel } from './model/PostsModel';
import { PostService } from './service/post.service';
import { Component, OnInit } from '@angular/core';
import { UserProfileService } from './service/user-profile.service';
import { UserProfileModel } from './model/UserProfileModel';

@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css']
})
export class AppComponent implements OnInit {
  title = 'social-media';

  getAllPosts:PostsModel[] = [];
  userProfiles:UserProfileModel[] = [];
  constructor(private postService:PostService,private userService:UserProfileService) { }

  ngOnInit(): void {
    this.userService.getAllUsers().subscribe(data=>{
      this.userProfiles = data as UserProfileModel[];
    });
  }


  slides = [
    {img: "data:image/png;base64,"},
    {img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTI0US1E7MbQ8bk_ldWZscbHq-5POuXVbEXdg&usqp=CAU"},
    {img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTxjM2T7lKzPEGPYJ2oaTr16462n8C6yolS3w&usqp=CAU"},
    {img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQ1ciXJepEEiaH5j0Ey7T0zIw_uhkG6tpB5Hw&usqp=CAU"}
  ];
  slideConfig = {"slidesToShow": 2, "slidesToScroll": 1, "autoplay":true ,"autoplaySpeed":5000
,"centerMode":true,

  }

  addSlide() {
    this.slides.push({img: "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTI0US1E7MbQ8bk_ldWZscbHq-5POuXVbEXdg&usqp=CAU"})
  }

  removeSlide() {
    this.slides.length = this.slides.length - 1;
  }

  slickInit(e) {
    console.log('slick initialized');
  }

  breakpoint(e) {
    console.log('breakpoint');
  }

  afterChange(e) {
    console.log('afterChange');
  }

  beforeChange(e) {
    console.log('beforeChange');
  }
}
